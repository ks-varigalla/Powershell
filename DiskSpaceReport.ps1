...
#$servers = Get-Content -Path "C:\Temp\Diskcheck-servers.txt" 
...
$result = Get-CimInstance -ComputerName $server -ClassName Win32_LogicalDisk | select @{Name="ServerName";Expression={$server}}, DeviceID, VolumeName, @{Name="Size(GB)";Expression={[Math]::Ceiling($_.size/1gb)}}, 

@{Name="Freespace(GB)";Expression={[Math]::Ceiling($_.freespace/1gb)}}, @{Name="UsedSpace(GB)";Expression={[Math]::Ceiling(($_.size - $_.freespace)/1GB)}},

@{Name="Used(%)";Expression={[Math]::Ceiling(($_.size - $_.freespace)/$_.size * 100)}}
