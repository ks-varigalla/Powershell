...
$report.Cells.Item(1,14) = "Company"
...
$op = Get-ADUser -filter * -Properties name,manager,samaccountname,company,title,country,department,office,mail,description,enabled,whencreated,accountexpirationdate,lastlogondate
...
foreach($rec in $op){
    ...
    if(($rec.SamAccountName.StartsWith('esc_')) -or ($rec.SamAccountName.StartsWith('adm_'))){
        ...
        if($man.manager -ne $null){
            ...
            $var2  = $man.Manager -split ',' | select -First 2

            if($var2.Split('=')[1] -ne $null){
            ...
            $report.Cells.Item($row,14) = $var2.Split('=')[1]
                ...
                $report.Cells.Item($row,14) = $var2.Split('=')[2]

            }

            $report.Cells.Item($row,15) = $rec.Company
        ...
        if($man.Count -gt 2){
            ...
            $report.Cells.Item($row,14) = "Multiple accounts exists"

            $report.Cells.Item($row,14).Interior.ColorIndex  = 4
...
$Workbook._SaveAs('NnNew_AD_ESC_ADM_Users_List.xlsx')
